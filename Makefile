...
PYTHONPKGS+=python-flower-srpm
PYTHONPKGS+=python-pandas-srpm
PYTHONPKGS+=python-pyhs2-srpm
...
airflowrepo-7-x86_64.cfg:: airflowrepo-7-x86_64.cfg.in
	sed "s|@@@REPOBASEDIR@@@|$(REPOBASEDIR)|g" $? > $@

airflowrepo-7-x86_64.cfg:: FORCE
	@diff -u $@ /etc/mock/$@ || \
		(echo Warning: /etc/mock/$@ does not match $@, exiting; exit 1)
...
python-celery-srpm:: python-billiard-srpm
...
python-flower-srpm:: python-tornado-srpm
...
python-pyhs2-srpm:: python-sasl-srpm
...
python-sphinx-srpm:: python-pygments-srpm
...
python-librabbitmq-srpm:: python-amqp-srpm
...
$(PYTHONPKGS):: airflowrepo-7-x86_64.cfg
